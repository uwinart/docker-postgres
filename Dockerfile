...
RUN echo "host    all             all             192.168.59.0/24            trust" >> /etc/postgresql/9.4/main/pg_hba.conf && \
  ...
  sed -i -e "s/shared_buffers\s*=.*/shared_buffers = 1024MB/g" /etc/postgresql/9.4/main/postgresql.conf && \
  sed -i -e "s/#effective_cache_size\s*=.*/effective_cache_size = 2048MB/g" /etc/postgresql/9.4/main/postgresql.conf && \
  sed -i -e "s/#maintenance_work_mem\s*=.*/maintenance_work_mem = 256MB/g" /etc/postgresql/9.4/main/postgresql.conf && \
  sed -i -e "s/#work_mem\s*=.*/work_mem = 2MB/g" /etc/postgresql/9.4/main/postgresql.conf && \
  sed -i -e "s/max_connections\s*=.*/max_connections = 1000/g" /etc/postgresql/9.4/main/postgresql.conf && \
